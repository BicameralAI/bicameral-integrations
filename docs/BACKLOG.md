...
- [ ] [B13] **Scorecard Token-Permissions hardening** (surfaced once the Scorecard gate actually ran, 2026-06-04): findings #21-24 (HIGH) — workflows grant write scopes at the top level (`codeql.yml`/`scorecard.yml` top-level `security-events: write`; `sbom.yml`/`_reusable-sbom.yml` `contents: write`). Move write scopes to job level with top-level `permissions: contents: read` to satisfy Scorecard's Token-Permissions check. Also disposition Pinned-Dependencies #18-20 (medium; `pip` not hash-pinned — the existing "accept" disposition for the stdlib-only dev toolchain) and dismiss the stale CodeQL #17 once a github-connector touch re-analyzes it. **Agreed as the next cycle after verify-wiring.**
