...
- [x] [B6] **OpenSSF Scorecard `startup_failure` on `main`** — Complete (2026-06-04). **Root cause corrected** (the prior "read-only token" theory was wrong — CodeQL + Security Scan both use `security-events: write` and pass, so the repo permits that scope): the failure was the **`id-token: write` OIDC request** for `publish_results: true` (publish to the public OpenSSF API), which the repo/org token policy refuses → GitHub rejects the run at startup (0–2 s "workflow file issue"). **Fixed in-workflow** (no admin): `publish_results: false` + dropped `id-token: write` in `scorecard.yml`/`_reusable-scorecard.yml`; the full analysis still runs and its SARIF still uploads to code-scanning via `security-events: write`. Only the public scorecard.dev badge publish is lost (not enrolled — see CII-Best-Practices disposition). Alternative if the public badge is ever wanted: repo-admin enables Actions OIDC (`id-token`) and reverts `publish_results`.
