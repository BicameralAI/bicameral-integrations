...
- [ ] [B15] **Vendored v1 ingest schema drift-recheck** (audit F-6): `runtime/schemas/ingest_request_v1.schema.json` is pinned from `bicameral-bot` commit `4f077998…` (2026-06-05). Re-verify the mapping (`runtime/gateway_mapping.py`) + re-pin when the bot publishes a v2 / changes required fields, so the offline conformance test doesn't pass against a stale contract. Live emission unblocked (bot #109 CLOSED, PR #131) — `GatewaySink` is real; an operator wiring `GatewaySink(endpoint, token)` against a real gateway is what promotes a connector to Live.
