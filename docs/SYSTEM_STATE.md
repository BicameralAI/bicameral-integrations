...
- [x] **Granola L2 connector-correction + descriptor (2026-06-11, Entry #137, L2)**: re-pointed identity off the non-existent `attendees` (live = `owner`); owner identity DROPPED (author="", PII-safe); transcript + title redact-and-passed; fixture updated to the verified live shape. Authored granola config.json (flip-ready). 7 of 26 connectors flip-ready. Realizes SG-2026-06-11-D.
...
- [x] **Provider Acquisition Contract — discovery edge documented (2026-06-16, Entry #205, L1)**: ADR-0017 (Proposed) + the consumable spec `docs/PROVIDER_ACQUISITION_CONTRACT.md` answer #173's source-acquisition boundary; ADR-0004/0006/0008/0011/0015 cross-linked to 0017; GOVERNANCE_INDEX registered. The discovery surface itself (`DiscoveryConnector` Protocol, `ProviderResourceDescriptor`/`ProviderItemEnvelope`, one FX-SEC-001 screen across pull+push, `create` via ProposedAction) is a net-new GAP awaiting **bot #405** sign-off before any code build (Drive `files.list` is the critical path). Docs/governance only; no `src/` touched. GH #173 answered on-issue; PR #174.
