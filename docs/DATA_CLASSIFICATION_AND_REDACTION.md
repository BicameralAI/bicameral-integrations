...
- **Redact-and-pass** — `adapter/core/redaction.py::redact(text)` scrubs the FX-SEC-001 catalog classes
  (value-consuming `sensitive.redact_catalog`) **plus email and phone (NANP 3-3-4; international with a
  `+` or `00` dialing prefix; and a keyword-anchored national run such as `tel:`/`call`/`mobile` — a bare
  national number with no prefix and no keyword is a documented residual)** to irreversible placeholders, so
...
label-adjacent); payment PANs (Luhn); email; phone (NANP + `+`/`00` international + keyword-anchored national; bare prefix-less national is a residual). **Not auto-detected (dropped at parse by the connector
