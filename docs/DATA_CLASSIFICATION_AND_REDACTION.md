...
- **Redact-and-pass** — `adapter/core/redaction.py::redact(text)` scrubs the FX-SEC-001 catalog classes
  (value-consuming `sensitive.redact_catalog`) **plus email and phone (NANP 3-3-4 + E.164/international
  `+CC` formats — best-effort, not exhaustive of every national grouping)** to irreversible placeholders, so
...
label-adjacent); payment PANs (Luhn); email; phone (NANP + E.164/international `+CC`, best-effort). **Not auto-detected (dropped at parse by the connector
