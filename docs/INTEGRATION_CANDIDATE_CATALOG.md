...
| Devin / Devin Desktop (Cognition) | P1 | Agentic-coder session evidence: goals, status, message trail, linked PRs, structured output, ACUs | Read-only Devin **v3 REST API** (sessions/messages, poll — no webhooks) | T1 | Researched (B7, 2026-06-04). Meets the bar via the **API** (versioned, RBAC read scope); the Desktop/Local path has **no documented file artifact**. Read session evidence = adapter here; launch/steer = `bicameral-mcp` (T3/T5). Build deferred behind Claude Code (P0). Risk: v3 schema churn + message-body redaction. See `docs/research-brief-devin-2026-06-04.md`. |
