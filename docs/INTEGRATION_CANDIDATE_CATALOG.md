...
| GitLab | P1 | Merge requests, issues, pipelines, code review evidence | Read-first, webhook ingest | T1/T3 | Important for self-hosted and enterprise teams. **BUILT → Beta (2026-06-05, `connectors/gitlab`)** — `parse_merge_request`/`parse_issue`; plaintext `X-Gitlab-Token` verify. |
...
| GitHub Copilot | P1 | AI-leverage usage/acceptance metrics by org/enterprise/team | Read-only API | T1 | Official usage-metrics endpoints (post-2026-04 set); per-user data is PII; needs admin permission + policy enabled. **BUILT → Beta (2026-06-05, `connectors/copilot`)** — `parse_metrics_day` over the aggregate `/copilot/metrics` object (PII-free); per-user NDJSON report deferred. |
| Cursor | P1 | Per-user activity, model/token usage, spend, audit logs | Read-only Admin API (read endpoints only) | T1 | Verify Privacy-Mode effect on the usage surface; admin-only key custody. **BUILT → Beta (2026-06-05, `connectors/cursor`)** — `parse_usage_day` aggregate metrics; drops `email`/`name`/`userId` at parse (no FX-SEC-001 email backstop, SG-2026-06-05-A). |
...
| Devin / Devin Desktop (Cognition) | P1 | Agentic-coder session evidence: goals, status, message trail, linked PRs, structured output, ACUs | Read-only Devin **v3 REST API** (sessions/messages, poll — no webhooks) | T1 | Researched (B7, 2026-06-04). Meets the bar via the **API** (versioned, RBAC read scope); the Desktop/Local path has **no documented file artifact**. Read session evidence = adapter here; launch/steer = `bicameral-mcp` (T3/T5). **BUILT → Beta (2026-06-05, `connectors/devin`)** — `parse_session` over v3 sessions; free-text passed through `redact()`. Risk: v3 schema churn. See `docs/research-brief-devin-2026-06-04.md` + `docs/research-brief-pii-redaction-devin-servicenow-2026-06-05.md`. |
...
| Confluence Cloud | P1 | Enterprise docs, RFCs, architecture records, runbooks | Read-first, webhook ingest | T1/T3 | Important for Atlassian shops. **BUILT → Beta (2026-06-05, `connectors/confluence`)** — `parse_content` over the REST storage body; verify deferred (Cloud signature unverifiable from docs — Data-Center-only `X-Hub-Signature`). |
...
| Zendesk | **P1** | Support tickets, CSAT, SLA-breach events, escalations | **Webhook-first** (Base64 HMAC-SHA256 over `timestamp+body`, `x-zendesk-webhook-signature` + timestamp), REST `/api/v2` backfill | T1/T5 | **BUILT — Beta** (`connectors/zendesk/`, FX-ZENDESK-001 + FX-WHSEC-003; `parse_ticket` subject-only excerpt + `verify_zendesk_signature`). Live REST/OAuth + the **PII redaction-and-pass model** for live ticket-body ingest deferred (`auth.md`). (eval + build 2026-06-04) |
| ServiceNow | P2 | ITSM incidents / changes / problems, change-governance evidence | Read-first poll (Table API `/api/now/table`, versioned; OAuth/Basic) | T1/T5 | **Poll-only — no portable signed-webhook contract** (outbound is bespoke per-tenant Business Rules); per-tenant customized. Scope a read-only role. **BUILT → Beta (2026-06-05, `connectors/servicenow`)** — `parse_incident` over the Table API; `description` through `redact()` (redact-and-pass), `caller_id` dropped. (eval 2026-06-04) |
