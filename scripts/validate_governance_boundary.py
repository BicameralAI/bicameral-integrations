...
def candidate_paths() -> list[str]:
    ...
    pr = git_lines(
        ["diff", "--name-only", "--diff-filter=ACMRT", f"origin/{base}...HEAD"]
    )
...
def check_registry_gitignore_agreement() -> list[str]:
    """Rule #4: every registered/floor root must be gitignored."""
    lines = gitignore_lines()
    problems = []
    for root in [*DEFAULT_FLOOR, *registry_roots()]:
        if not is_covered_by_gitignore(root, lines):
            problems.append(
                f"registered sibling root not covered by .gitignore: {root}"
            )
    return problems
...
def main() -> int:
    ...
    for path in paths:
        ...
        if root is not None:
            problems.append(
                f"local/sibling artifact must not be committed: {path}"
                f"  (matches `{root}`)"
            )
