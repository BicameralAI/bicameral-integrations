...
one) — and fail-closed: it re-runs the producer screen and requires a complete
redaction receipt at the emission boundary, accepts only HTTP 201, and raises
``GatewayEmissionError`` on any other status or transport fault. Read-only
evidence boundary (ADR-0008); the auth token is operator-injected and never
...
class GatewayRedactionGated(RuntimeError):
    """Raised when a Bot-bound emission lacks a valid redaction receipt."""
...
def _require_redaction_receipt(emission: AdapterEmission) -> None:
    ...
    if receipt.get("structural_fields_preserved") is not True:
        raise GatewayRedactionGated("gateway emission gated: structural_fields_not_preserved")
    ...
    if not isinstance(receipt.get("findings"), list):
        raise GatewayRedactionGated("gateway emission gated: redaction_findings_invalid")
