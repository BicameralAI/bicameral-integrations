...
"""Model-free hosted validation for committed ADR-0020 evaluation evidence.

The workflow asserts the real checkout SHA before calling this module. This
module validates the supplied exact-head identity plus the committed artifact
set without spawning external processes or provisioning candidate models.
"""
...
def _normalize_expected_head(expected_head: str | None, errors: list[str]) -> str | None:
...
def _gate_state(
    ...
    for index, gate in enumerate(gate_items):
        ...
        if status == "pending" and isinstance(gate_id, str):
            pending_ids.append(gate_id)
        if status == "failed" and isinstance(gate_id, str):
    ...
    return all_passed, sorted(pending_ids), sorted(failed_ids)
...
def validate_repository(
    ...
    *,
    ...
    current_head = _normalize_expected_head(expected_head, errors)
    ...
    for candidate_id in sorted(set(matrix) | set(gates) | set(metrics)):
        ...
        all_passed, pending_ids, failed_ids = _gate_state(candidate_id, gate_entry, errors)
