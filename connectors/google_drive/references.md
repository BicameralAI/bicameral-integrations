...
| Readiness (lifecycle) | Beta → **flip-ready, NOT yet Live** (documents.get active fetch, proven end-to-end through the `runtime/` harness against a reference sink; ADR-0012). The Live flip is gated on operator human review + a live test against the real Google Docs API with a real OAuth credential — still incomplete until then. |
