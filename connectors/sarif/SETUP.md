...
- Emits: finding
- PII posture: The SARIF schema carries no user PII by design, but a SECURITY-SCANNER finding's message.text can quote the very secret it flags (a secret-scanner emits 'Detected AWS key AKIA... in config.py'). So message.text is redact-and-passed -- redact() scrubs secret/PHI/PAN + email/phone. This is the security-correct choice (SG-2026-06-13-E): emitted RAW, FX-SEC-001 would HARD-REJECT the finding and the security signal would be LOST; redact-and-pass scrubs the secret VALUE and PRESERVES the finding ('Detected AWS key [redacted:secret] in config.py'). The connector reads the finding MESSAGE only, NEVER the raw code snippet (region.snippet.text) -- data minimization. The ruleId/ref floor (rule id + repo path) is not redacted. FX-SEC-001 hard-screens secret/PHI/PAN as the un-bypassable backstop.
