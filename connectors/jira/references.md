...
| Readiness (lifecycle) | Beta -> **flip-ready, NOT yet Live** (parse + `X-Hub-Signature` verify + dedup; summary redact-and-passed, ADF description never read, actor identity dropped; FX-CFG-001 webhook descriptor shipped). Webhook receipt operator-runtime; Connect-JWT/Forge/active deferred. Live flip gated on a signed live delivery. |
