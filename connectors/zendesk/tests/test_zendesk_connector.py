...
def test_parse_ticket_maps_subject_and_metadata():
    ...
    assert obs.excerpt.startswith("Login fails after password reset")
...
def test_ticket_body_redacted_and_emitted():
    obs = parse_ticket(_event())
    assert "Customer cannot log in" in obs.excerpt
...
def test_raw_body_is_sanitized_by_universal_boundary():
    raw = _event()["detail"]["description"]
    observation = Observation(
        source_ref=SourceRef(source_id="zendesk", ref="x"),
        excerpt=raw,
    )
    emission = normalize([observation], adapter_version="x")[0]
    assert "user@example.com" not in emission.body
    assert "AKIAABCDEFGHIJKLMNOP" not in emission.body
    categories = {
        finding["category"]
        for finding in emission.metadata["redaction_receipt"]["findings"]
    }
    assert {"pii", "secret"} <= categories
    normalize(ZendeskConnector().observations(_event()), adapter_version="x")


def test_subject_only_when_no_body():
    event = {
        "subject": "zen:ticket:8899",
        "detail": {"id": "8899", "subject": "Billing question"},
    }
    obs = parse_ticket(event)
