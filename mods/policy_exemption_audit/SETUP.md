...
# Policy Exemption Audit — mod setup

Advisory mod that surfaces evidence claiming a policy exemption/waiver/accepted-risk (exempt/waiver/wontfix/accepted risk/policy exception/...) so a policy owner can re-approve it.

- **id** `policy_exemption_audit` · **manifest** `policy-exemption-audit` · **family** governance · **version** 0.1.0 · **channel** beta
...
Policy-exemption/waiver/accepted-risk CLAIMS (exempt/waiver/grandfathered/wontfix/accepted risk/policy exception/suppress finding/...) in title + body + evidence excerpts.
...
```bash
python -m runtime.cli run-mods <connector> --mods policy_exemption_audit
```
...
- scope: mods/policy_exemption_audit/README.md
