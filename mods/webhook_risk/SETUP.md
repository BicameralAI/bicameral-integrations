...
# Webhook Risk — mod setup

Advisory mod that flags webhook safety signals across signature verification, replay protection, event schema handling, idempotency, and outbound side effects.

- **id** `webhook_risk` · **manifest** `webhook-risk` · **family** security · **version** 0.1.0 · **channel** beta
...
Webhook safety signals across signature verification, replay protection, event schema handling, idempotency, and outbound side effects.

## Reads (evidence consumed)

- webhook evidence (signature/replay/schema signals)
- source_ref (kind=webhook)
...
```bash
python -m runtime.cli run-mods <connector> --mods webhook_risk
```
...
- scope: mods/webhook_risk/README.md
